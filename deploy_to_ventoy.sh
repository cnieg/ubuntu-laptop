...
if [ -f "logo_oasis.png" ]; then
    echo "Copie de logo_oasis.png vers oasis-logo.png..."
    cp logo_oasis.png "$VENTOY_PATH/ventoy/ubuntu-autoinstall/oasis-logo.png"
elif [ -f "$HOME/logo_oasis.png" ]; then
    echo "Copie de logo_oasis.png vers oasis-logo.png..."
    cp "$HOME/logo_oasis.png" "$VENTOY_PATH/ventoy/ubuntu-autoinstall/oasis-logo.png"
else
    echo "⚠️  logo_oasis.png non trouvé (optionnel)"
fi
...
autoinstall:
  ...
  late-commands:
    # Créer le dossier et copier le logo depuis plusieurs emplacements possibles
    - mkdir -p /target/usr/share/pixmaps
    
    # Ventoy place les fichiers dans /run/initramfs/overlayfs/media/
    # Essayer plusieurs emplacements
    - cp /run/initramfs/overlayfs/media/oasis-logo.png /target/usr/share/pixmaps/oasis-logo.png 2>/dev/null || true
    - cp /cdrom/oasis-logo.png /target/usr/share/pixmaps/oasis-logo.png 2>/dev/null || true
    - cp /media/*/oasis-logo.png /target/usr/share/pixmaps/oasis-logo.png 2>/dev/null || true
    
    # Définir les bonnes permissions
    - chmod 644 /target/usr/share/pixmaps/oasis-logo.png 2>/dev/null || true
...
echo "          ├── user-data"
echo "          ├── meta-data"
echo "          └── oasis-logo.png (si présent)"
echo ""
